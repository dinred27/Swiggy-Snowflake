...
def fetch_kpi_data():
    ...
    SELECT 
        ...
        max_order_value
    FROM swiggy.consumption_sch.vw_yearly_revenue_kpis
    ORDER BY year;
    """
...
def fetch_monthly_kpi_data(year):
    ...
    FROM 
    swiggy.consumption_sch.vw_monthly_revenue_kpis
    WHERE year = {year}
    ORDER BY month;
    """
    return session.sql(query).collect()
...
def fetch_unique_months(year):
    ...
    SELECT 
        DISTINCT MONTH FROM 
    swiggy.consumption_sch.vw_monthly_revenue_by_restaurant 
    WHERE YEAR = {year} 
    ORDER BY MONTH;
    """
    return session.sql(query).collect()
